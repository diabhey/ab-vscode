...
class RepoLogProvider {
    constructor(sourceControlManager) {
        ...
        this._dispose = [];
        this.refresh();
        this._dispose.push(vscode_1.commands.registerCommand("svn.repolog.copymsg", async (item) => common_1.copyCommitToClipboard("msg", item)));
    ...
    async openDiffCmd(element) {
        ...
        let prevRev;
        {
            ...
            if (posPrev !== undefined) {
                ...
                const revs = await item.repo.log(parent.revision, "1", 2, nm.parse(commit._).remoteFullPath);
                if (revs.length === 2) {
                    prevRev = revs[1];
                }
                else {
